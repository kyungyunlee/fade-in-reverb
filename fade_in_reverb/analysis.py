...
def load_common_decay_times(filepath, rirs, n_slopes=3) : 
...
def compute_common_decay_times(rirs, n_slopes=3) : 
    common_decay_times =  [] 
...
def load_envelope_fit_result(filepath, rirs, common_decay_times, plot=False) : 
    if not os.path.exists(filepath) : 
        print (f"{filepath} does not exist. Need to perform fitting.")
        if not os.path.exists(os.path.dirname(filepath)): 
            os.makedirs(os.path.dirname(filepath), exist_ok=True) 
        pos_fit_result, neg_fit_result, all_original_env = envelope_fit(rirs, common_decay_times, plot)
...
def envelope_fit(rirs, common_decay_times, plot=False) : 
    ...
    L = rirs[0].shape[0] # Length of rir
    ...
    all_original_env = np.zeros((rirs.shape[0], len(config['f_bands']), downSampleLength - ds_start_index))
    neg_fit_result = np.zeros((rirs.shape[0], len(config['f_bands']), n_slopes+1))
    pos_fit_result = np.zeros((rirs.shape[0], len(config['f_bands']), n_slopes+1))
    ...
    for bIdx in range(len(config['f_bands'])) : 
        ...
        for rIdx in range(len(rirs)): 
            ...
            # Save for later 
            all_original_env[rIdx][bIdx] = original_env_ds * octave_filtered_rir_norm_factor
            ...
            neg_fit_result[rIdx][bIdx] = res_neg.x * octave_filtered_rir_norm_factor
            pos_fit_result[rIdx][bIdx] = res_pos.x * octave_filtered_rir_norm_factor
           
            if plot and rIdx >800 : 
                counter = 0 
                # Negative result 
                if counter < 2 : 
                    counter += 1 
